...
void allocate(DynamicArray ** v, unsigned int size);
...
int main (int argc, char *argv[]) {
	...
	allocate(&rand_data, info.max_size);
...
void allocate(DynamicArray **v, unsigned int size) {
	*v = malloc(sizeof(DynamicArray) * size);
	if (!*v) {
...
void resize(DynamicArray **v) {
	...
	allocate(&new_v, info.max_size);
	memcpy(new_v, *v, sizeof(DynamicArray) * info.curr_idx);
	destroy(v);
